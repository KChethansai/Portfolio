...
export function FlowSection({ className, style = {}, children, ['aria-label']: ariaLabel }) {
  return (
    <section data-flow-section aria-label={ariaLabel} className={cx('relative min-h-screen w-full overflow-hidden', className)}>
      <div data-flow-inner className={cx('flow-art-container relative flex min-h-screen w-full flex-col justify-between gap-6 px-[4vw] pb-[4vw] pt-[clamp(2rem,8vw,4vw)]', 'will-change-transform')} style={{ transformOrigin: 'bottom left', ...style }}>
        {children}
      </div>
    </section>
  )
...
export default function FlowArt({ children, className, ['aria-label']: ariaLabel = 'Story scroll' }) {
  ...
  }, [reducedMotion, React.Children.count(children)])
